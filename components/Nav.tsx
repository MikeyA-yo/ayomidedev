  ...
  return (
    <>
      <AnimatePresence>
        {!open && (
          <motion.div
            className="flex justify-end w-full flex-col"
            initial="opening"
            animate="opened"
            exit="closing"
          ...
          >
            {values.map((item, index) => {
              return (
                <NavElements key={index} name={item.name} link={item.link} />
              );
            })}
          </motion.div>
        )}
...
export default function Navbar() {
  ...
  return (
    <>
      <div
        className={`w-full h-20 p-4 fixed md:flex lg:flex items-center z-50  hidden justify-between gap-40 bg-white`}
      >
        <div className="flex-grow">
          <Link href={"/"}>
            <p className={`text-xl text-violet-700 ${ub}`}>AYOMIDE</p>
          </Link>
        </div>
